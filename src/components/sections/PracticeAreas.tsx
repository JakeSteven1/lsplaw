...
export default function PracticeAreas() {
  return (
    <section id="practice-areas" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
      <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight">Practice Areas</h2>
      <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {practiceAreas.map(item => {
          const Icon = iconMap[item.icon] ?? ScaleIcon
          return (
            <div key={item.name} className="rounded-2xl border border-gray-200 p-6 shadow-sm bg-white/70 dark:border-white/10 dark:bg-white/5">
